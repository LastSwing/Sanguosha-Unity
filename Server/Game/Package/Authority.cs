...
namespace SanguoshaServer.Package
{
    ...
    public abstract class ImperialOrder : FunctionCard
    {
        ...
        public override void OnUse(Room room, CardUseStruct card_use)
        {
            ...
            CardMoveReason reason = new CardMoveReason(CardMoveReason.MoveReason.S_REASON_ANNOUNCE, player.Name, null, card_use.Card.Skill, null)
            {
    ...
    {
        public BeltsheChao() : base("BeltsheChao") { }
    ...
    public class Surrender : ImperialOrder
    {
        ...
        public override bool Effect(Room room, Player player, Player target)
        {
            ...
            if (throw_ids.Count > 0)
            {
                ...
                room.ThrowCard(ref throw_ids, new CardMoveReason(CardMoveReason.MoveReason.S_REASON_THROW, target.Name, target.Name, "surrender", "surrender"), target);
    ...
    {
        public SurrenderCard() : base("SurrendCard")
        {
        }
    ...
    {
        public PayTribute() : base("PayTribute") { }
    ...
    {
        public StopFighting() : base("StopFighting") { }
        ...
        public override void Record(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            if (data is PhaseChangeStruct change && change.To == PlayerPhase.NotActive)
            {
                ...
                if (player.GetMark("stopfighting") > 0)
                {
                    ...
                    room.RemovePlayerStringMark(player, "StopFighting");
                }
            }
        }
    ...
    public class Jieyue : PhaseChangeSkill
    {
        ...
        {
            WrappedCard card = room.AskForUseCard(player, "@@jieyue", "@jieyue", -1, FunctionCard.HandlingMethod.MethodNone, true, info.SkillPosition);
            if (card != null)
                return info;
    ...
    public class JieyueVS : OneCardViewAsSkill
    {
        ...
        public override WrappedCard ViewAs(Room room, WrappedCard card, Player player)
        {
            WrappedCard jieyue = new WrappedCard("JieyueCard");
    ...
    public class JieyueCard : SkillCard
    {
        ...
        public override void OnUse(Room room, CardUseStruct card_use)
        {
            ...
            room.NotifySkillInvoked(player, "jieyue");
            room.DoAnimate(AnimateType.S_ANIMATE_INDICATE, player.Name, target.Name);
            LogMessage log = new LogMessage
            {
            ...
            room.ObtainCard(target, new List<int>(card_use.Card.SubCards),
                new CardMoveReason(CardMoveReason.MoveReason.S_REASON_GIVE, player.Name, target.Name, "jieyue", "jieyue"), false);
        }
    }
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            if (target != null)
            {
                if (target.HasFlag(Name))
                {
                    ...
                    {
                        WrappedCard card = room.AskForUseCard(target, "@@zhengpi", "@zhengpi-give");
                        if (card != null)
    ...
    public class WeidiVS : ViewAsSkill
    {
        ...
        public override WrappedCard ViewAs(Room room, List<WrappedCard> cards, Player player)
        {
            ...
            {
                WrappedCard card = new WrappedCard("WeidiCard")
                {
    ...
    {
        public WeidiCard() : base("WeidiCard") { }
    ...
    public class Yongsi : TriggerSkill
    {
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (base.Triggerable(player, room) && data is CardUseStruct use && use.Card != null && use.To.Contains(player) && use.Card.Name == "KnownBoth")
                return new TriggerStruct(Name, player);
    ...
    public class YongsiVH : ViewHasSkill
    {
        ...
        {
            viewhas_treasure.Add("JadeSeal");
        }

        public override bool ViewHas(Room room, Player player, string skill_name)
        {
            if (skill_name == "JadeSeal" && RoomLogic.PlayerHasShownSkill(room, player, "yongsi"))
            {
                bool in_game = false;
                foreach (Player p in room.GetAlivePlayers())
                {
    ...
    {
        public XuanhuoCard() : base("XuanhuoCard")
        {
            target_fixed = true;
        }
    ...
    {
        public JianglueCard() : base("JianglueCard") { target_fixed = true; }
        public override void OnUse(Room room, CardUseStruct card_use)
        {
    ...
    {
        public GanluCard() : base("GanluCard") { }
        ...
        public override void Use(Room room, CardUseStruct card_use)
        {
            ...
            CardsMoveStruct move2 = new CardsMoveStruct(equips2, first, Place.PlaceEquip,
                new CardMoveReason(CardMoveReason.MoveReason.S_REASON_SWAP, second.Name, first.Name, "ganlu", string.Empty));
    ...
    public class Buyi : TriggerSkill
    {
        ...
        public override List<TriggerStruct> Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data)
        {
            ...
            if (triggerEvent == TriggerEvent.QuitDying && player.Alive && data is DamageStruct damage && damage.From != null && damage.From.Alive)
            {
    ...
    public class KeshouVS : ViewAsSkill
    {
        ...
        public override WrappedCard ViewAs(Room room, List<WrappedCard> cards, Player player)
        {
            if (cards.Count == 2)
            {
                WrappedCard card = new WrappedCard("KeshouCard");
                card.AddSubCards(cards);
                return card;
            }
    ...
    {
        public KeshouCard() : base("KeshouCard") { target_fixed = true; }
    ...
    public class Zhuwei : TriggerSkill
    {
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (triggerEvent == TriggerEvent.FinishJudge && data is JudgeStruct judge
                ...
                && (judge.Card.Name.Contains("Slash") || judge.Card.Name == "Duel"))
                return new TriggerStruct(Name, player);
    ...
    public class Huibian : ZeroCardViewAsSkill
    {
        ...
        public override WrappedCard ViewAs(Room room, Player player)
        {
            WrappedCard card = new WrappedCard("HuibianCard")
            {
                Skill = Name,
                ShowSkill = Name
            };
    ...
    {
        public HuibianCard() : base("HuibianCard") { }
    ...
    public class Zongyu : TriggerSkill
    {
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (triggerEvent == TriggerEvent.CardsMoveOneTime && data is CardsMoveOneTimeStruct move && move.To != null && move.To_place == Place.PlaceEquip)
            {
                ...
                {
                    if (room.GetCard(id).Name == "DragonCarriage")
                    {
                        check = true;
                        break;
                    }
            ...
            else if (triggerEvent == TriggerEvent.CardUsed && base.Triggerable(player, room) && data is CardUseStruct use)
            {
                ...
                if (fcard is Horse)
                {
                    ...
                    foreach (int card_id in room.DiscardPile)
                        if (room.GetCard(card_id).Name == "DragonCarriage")
                            return new TriggerStruct(Name, player);
                }
            }
            ...
            if (triggerEvent == TriggerEvent.CardsMoveOneTime && data is CardsMoveOneTimeStruct move && move.To != null && move.To_place == Place.PlaceEquip)
            {
                ...
                if (first_ids.Count > 0 && second_ids.Count > 0)
                {
                    ...
                    CardsMoveStruct move1 = new CardsMoveStruct(first_ids, ask_who, Place.PlaceEquip,
                        new CardMoveReason(CardMoveReason.MoveReason.S_REASON_SWAP, move.To.Name, ask_who.Name, Name, string.Empty));
                    CardsMoveStruct move2 = new CardsMoveStruct(second_ids, move.To, Place.PlaceEquip,
                        new CardMoveReason(CardMoveReason.MoveReason.S_REASON_SWAP, ask_who.Name, move.To.Name, Name, string.Empty));
                    exchangeMove.Add(move2);
                    exchangeMove.Add(move1);
    ...
    public class Jianan : TriggerSkill
    {
        ...
        public override TriggerStruct Triggerable(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who)
        {
            if (player.Phase == PlayerPhase.Start && player.HasShownOneGeneral())
            {
                Player lord = RoomLogic.FindPlayerBySkillName(room, Name);
                if (RoomLogic.IsFriendWith(room, player, lord))
                    return new TriggerStruct(Name, player);
            }
    ...
    public class QiaobianCC : TriggerSkill
    {
        ...
        public override bool Effect(TriggerEvent triggerEvent, Room room, Player player, ref object data, Player ask_who, TriggerStruct info)
        {
            ...
            if (index == 2 || index == 3)
            {
                ...
                room.AskForUseCard(player, "@@qiaobian", use_prompt, index, FunctionCard.HandlingMethod.MethodUse, true, info.SkillPosition);
            }
            return false;
        }
    }
