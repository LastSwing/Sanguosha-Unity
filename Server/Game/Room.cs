...
namespace SanguoshaServer.Game
{
    ...
    public class Room : IDisposable
    {
        ...
        public void DrawCards(List<Player> players, List<DrawCardStruct> reasons)
        {
            ...
            for (int i = 0; i < players.Count; i++)
            {
                ...
                if (reason.Who != null && player != reason.Who && RoomLogic.PlayerHasSkill(this, reason.Who, reason.Reason)
                    && (RoomLogic.WillBeFriendWith(this, player, reason.Who) || RoomLogic.WillBeFriendWith(this, reason.Who, player, reason.Reason)))
                {
                ...
                RoomThread.Trigger(TriggerEvent.XuyouDraw, this, player, ref data);
        ...
        public void Recover(Player player, RecoverStruct recover_struct, bool set_emotion = false)
        {
            ...
            object _lose = num;
            if (!RoomThread.Trigger(TriggerEvent.HpChanged, this, player, ref _lose))
            {
                player.Hp = new_hp;
            ...
            for (int i = 0; i < cards_moves.Count; i++)
            {
                ...
                for (int j = 0; j < cards_move.Card_ids.Count; j++)
                {
                    ...
                    if (cards_move.From_place == Place.PlaceHand || cards_move.From_place == Place.PlaceEquip)
                        System.Diagnostics.Debug.Assert(cards_move.From != null, card_id.ToString());
                    if (cards_move.From != null) // Hand/Equip/Judge
                    {
                        RoomLogic.RemovePlayerCard(this, FindPlayer(cards_move.From, true), card_id, cards_move.From_place);
                    }
        ...
        public bool OnClientRequestInter(Client client, int id, string pwd)
        {
            ...
            lock (this)
            {
                ...
                client.CheatArgs = new List<string>();
        ...
        public void OnHostInter()
        {
            lock (this)
            {
                ...
                Host.CheatArgs = new List<string>();
        ...
        private void OnClientDisconnected(object sender, EventArgs e)
        {
            lock (this)
            {
                ...
                else
                {
                    ...
                    foreach (Player p in GetPlayers(client))
                    {
                        p.Status = "offline";
        ...
        private void ProcessClientReply(Client player, MyData data)
        {
            lock (this)
            {
                ...
                if (!player.IsWaitingReply || player.IsClientResponseReady)
                    Debug(string.Format("Server is not waiting for reply from {0}", player.UserName));
                    ...
                    Debug(string.Format("Reply command should be {0} instead of {1}", player.ExpectedReplyCommand, reply_command));
        ...
        private void OnLuckCardReply(Client player)
        {
...

            {
                ...
                else
                {
                    ...
                    foreach (Player p in GetPlayers(player))
                    {
                        log.From = p.Name;
                        SendLog(log);
                    }
        ...
        private void Marshal(Client client)
        {
            ...
            //五谷丰登
            if (ContainsTag("AmazingGraceOrigin") && GetTag("AmazingGraceOrigin") is List<int> amazing && amazing.Count > 0)
                FillAG(AmazingGrace.ClassName, amazing, GetPlayers(client)[0]);
            //鏖战通知
            if (BloodBattle)
            {
            ...
            //装备、手牌
            List<Player> _players = new List<Player> { GetPlayers(client)[0] };
            foreach (Player player in m_players)
            {
        ...
        public void KingdomSummons(string kingdom)
        {
            ...
            if (no_shown.Count > 0)
            {
                ...
                foreach (Player player in no_shown)
                {
                    ...
                    {
                        bool success = true;
                        List<string> reply = GetClient(player).ClientReply;
                        if (!GetClient(player).IsClientResponseReady || reply == null || reply.Count == 0 || string.IsNullOrEmpty(reply[0]))
                            success = false;
        ...
        public void AskForLuckCard(int count)
        {
            ...
            foreach (Client player in Clients)
            {
                ...
                if (GetAI(GetPlayers(player)[0]) == null)
                {
                    players.Add(player);
                    player_luckcard[player] = count;
                }
            }
        ...
        public void LoseHp(Player victim, int lose = 1)
        {
            ...
            object _lose = -lose;
            if (!RoomThread.Trigger(TriggerEvent.HpChanged, this, victim, ref _lose))
            {
                victim.Hp -= lose;
        ...
        public bool ApplyDamage(Player victim, DamageStruct damage)
        {
            ...
            bool result = RoomThread.Trigger(TriggerEvent.HpChanged, this, victim, ref data);
            if (!result)
            {
            ...
            while (card == null)
            {
                ...
                if (player.Alive && card == null)
                {
                    ...
                    else
                    {
                        ...
                        List<string> clientReply = client != null && client.GameRoom == RoomId ? client.ClientReply : null;
                        if (client != null && success && clientReply != null && clientReply.Count != 0)
                        {
                         ...
                         string prompt = null, bool notify_skill = false, string position = null)
        {
            ...
            else
            {
                ...
                if (client != null && client.GameRoom == RoomId)
                {
                    List<string> clientReply = client.ClientReply;
        ...
        public bool UseCard(CardUseStruct card_use, bool add_history = true, bool ignore_rule = false)
        {
                    ...
                    && !Engine.CorrectCardTarget(this, TargetModSkill.ModType.History, card_use.From, null, card))
            {
                ...
                if (card.Name != key && card.Name.Contains(Slash.ClassName))
            ...
            HandlingMethod method = HandlingMethod.MethodUse, bool addHistory = true, string position = null)
        {
            ...
            if (card != null)
            {
                object decisionData = string.Format("cardUsed:{0}:{1}:{2}", pattern, prompt, RoomLogic.CardToString(this, card));
                RoomThread.Trigger(TriggerEvent.ChoiceMade, this, player, ref decisionData);
            }
            else
            {
        ...
        public WrappedCard AskForUseSlashTo(Player slasher, List<Player> victims, string prompt, bool addHistory = false)
        {
            ...
            WrappedCard slash = AskForUseCard(slasher, Slash.ClassName, prompt, -1, HandlingMethod.MethodUse, addHistory);
            if (slash == null)
            {
                slasher.SetFlags("-slashTargetFix");
                slasher.SetFlags("-slashTargetFixToOne");
        ...
        public bool _AskForNullification(WrappedCard trick, Player from, Player to, bool positive, _NullificationAiHelper helper)
        {
            ...
            if (repliedPlayer != null)
            {
                ...
                {
                    Client client = GetClient(repliedPlayer);
                    List<string> clientReply = client?.ClientReply;
                            ...
                            string skill_name = null, object data = null, bool can_convert = false, bool assign_kingdom = false)
        {
            ...
            else
            {
                ...
                if (clientResponse != null && clientResponse.Count > 0)
                {
                    ...
                    {
                        if (skill_name == "gamerule" && !generals.Contains(name) && !(generals.Contains(Engine.GetMainGeneral(name)) && can_convert)
                            && GetClient(player).UserRight < 3)
                        {
                            valid = false;
                            break;
                        }
                        else if (skill_name != "gamerule" && (!can_convert && !generals.Contains(name) || !generals.Contains(Engine.GetMainGeneral(name))))
                        {
                            valid = false;
                            break;
                        }
        ...
        */
            ...
            string expand_pile = null, bool notify_skill = true, string position = null)
        {
            ...
            do
            {
                ...
                else
                {
                    ...
                    bool success = client != null && client.YijiRequest(this, guojia, cards, players, prompt, max_num, optional, expand_pile, position);
            ...
            if (notify_skill)
            {
                ...
                if (skill != null) BroadcastSkillInvoke(_skill_name, "male", index, general, skin_id);
            ...
            int min_num, int max_num, bool can_refuse, bool write_step, List<int> notify_visible_list, string position)
        {
                    ...
                    if (!match)
                    {
                        ...
                        for (int i = 0; i < top_cards.Count; ++i)
                        {
                            ...
                            if (top_cards[i] != ups[i])
                            {
                                ...
                                List<string> movearg_base = new List<string> { GuanxingStep.S_GUANXING_MOVE.ToString(), target_id.ToString(), fromPos.ToString(), toPos.ToString() };
                        ...
                        if (ups.Count > top_cards.Count)
                        {
                            ...
                            for (int i = 1; i <= newcount; ++i)
                            {
                                ...
                                List<string> movearg_base = new List<string> { GuanxingStep.S_GUANXING_MOVE.ToString(), adjust_id.ToString(), fromPos.ToString(), toPos.ToString() };
                                DoBroadcastNotify(CommandType.S_COMMAND_MIRROR_MOVECARDS_STEP, movearg_base, isTrustAI ? null : GetClient(zhuge));
                                Thread.Sleep(500);
                            }
                        }
                        ...
                        for (int i = 0; i < bottom_cards.Count - 1; ++i)
                        {
                            ...
                            if (bottom_cards[i] != downs[i])
                            {
                                ...
                                List<string> movearg_base = new List<string> { GuanxingStep.S_GUANXING_MOVE.ToString(), bottom_cards[i].ToString(), fromPos.ToString(), toPos.ToString() };
                                DoBroadcastNotify(CommandType.S_COMMAND_MIRROR_MOVECARDS_STEP, movearg_base, isTrustAI ? null : GetClient(zhuge));
                                Thread.Sleep(500);
                            }
                        }
            ...
            else
            {
                ...
                if (client != null && client.GameRoom == RoomId && client.ClientReply != null && client.ClientReply.Count > 0)
        ...
        public AskForMoveCardsStruct AskforSortCards(Player player, string skillName, List<int> cards, bool visible, string position)
        {
            ...
            List<string> stepArgs = new List<string>
            {
                GuanxingStep.S_GUANXING_START.ToString(), player.Name, skillName, JsonUntity.Object2Json(cards),
                MoveCardType.S_TYPE_SORT.ToString(), position
            };
            ...
            if (ai != null)
            {
                ...
                AskForMoveCardsStruct map = ai.AskForSortCards(cards, skillName);
                top_cards = map.Top;
                bottom_cards = map.Bottom;
                ...
                if (success)
                {
                    foreach (int id in top_cards)
                    {
                        List<string> movearg_base = new List<string> { GuanxingStep.S_GUANXING_MOVE.ToString(), id.ToString(), "0", "1" };
                        DoBroadcastNotify(CommandType.S_COMMAND_MIRROR_MOVECARDS_STEP, movearg_base, isTrustAI ? null : GetClient(player));
                        Thread.Sleep(500);
                    }

                    foreach (int id in bottom_cards)
                    {
                        List<string> movearg_base = new List<string> { GuanxingStep.S_GUANXING_MOVE.ToString(), id.ToString(), "0", "2" };
                        DoBroadcastNotify(CommandType.S_COMMAND_MIRROR_MOVECARDS_STEP, movearg_base, isTrustAI ? null : GetClient(player));
                        Thread.Sleep(500);
                    }
            ...
            else
            {
                Client client = GetClient(player);
                if (client != null)
                    client.SortCardRequest(this, player, skillName, cards, position);
                AskForMoveCardsStruct guanxin = new AskForMoveCardsStruct
                {
                    Top = new List<int>(),
                    Bottom = new List<int>()
            ...
            DoBroadcastNotify(CommandType.S_COMMAND_MIRROR_MOVECARDS_STEP, new List<string> { GuanxingStep.S_GUANXING_FINISH.ToString() }, isTrustAI ? null : GetClient(player));
            DoBroadcastNotify(CommandType.S_COMMAND_UNKNOWN, new List<string> { false.ToString() });
        ...
        public int DoGongxin(Player shenlvmeng, Player target, List<int> enabled_ids, string skill_name, string prompt, string position)
        {
            Thread.Sleep(300);
            ...
            else
            {
                List<string> gongxinArgs = new List<string> {
                    ...
                    target.Name,
                    false.ToString(),
                    JsonUntity.Object2Json(target.HandCards),
                ...
                if (!success || clientReply == null || clientReply.Count == 0 || !target.HandCards.Contains(int.Parse(clientReply[0])))
                {
                }
                else
                    card_id = int.Parse(clientReply[0]);
